...
const DeepAnalysis = ({ attack, onClose }) => {
    ...
    <div className="analysis-section">
      ...
      <div className="overview-cards">
        <div className="overview-card profit">
          ...
          <div className="sub-text">${(parseFloat(attack.profit || 0) * 2500).toFixed(2)} USD</div>
        </div>
        ...
        <div className="overview-card gas">
          ...
          <div className="sub-text">{mevData.gasPrice} Gwei</div>
        </div>
      </div>
    </div>
  );
  ...
  const renderGraph = () => {
    ...
    const generatePriceData = () => {
      ...
      return [
        { time: 'Before Attack', price: basePrice, label: 'Normal Price' },
        { time: 'Front-run', price: basePrice + (basePrice * slippage / 200), label: 'Price Increase' },
        { time: 'Victim Tx', price: basePrice + (basePrice * slippage / 100), label: 'Peak Price' },
        { time: 'Back-run', price: basePrice + (basePrice * slippage / 300), label: 'Price Drop' },
        { time: 'After Attack', price: basePrice, label: 'Recovery' }
      ];
    };
          ...
          <div className="price-graph">
            ...
            <div className="graph-chart">
              <div className="y-axis">
                ...
                <div className="y-label mid">${((maxPrice + minPrice) / 2).toFixed(0)}</div>
                ...
                <svg viewBox="0 0 400 200" className="price-chart-svg">
                  ...
                  <polyline
                    points={priceData.map((d, i) => 
                      `${i * 80 + 40},${200 - ((d.price - minPrice) / priceRange) * 160 - 20}`
                    ).join(' ')}
                    fill="none"
                    stroke="#4F46E5"
                    strokeWidth="3"
              ...
              <div className="x-axis">
                {priceData.map((d, i) => (
                  <div key={i} className="x-label">{d.time}</div>
                ))}
              </div>
            </div>
            ...
            <div className="impact-metrics">
              <div className="metric">
                ...
                <div className="metric-value impact-text">{mevData.slippage}%</div>
              </div>
              <div className="metric">
                <div className="metric-label">Victim Loss</div>
                <div className="metric-value loss-text">{mevData.victimLoss} ETH</div>
              </div>
              <div className="metric">
                <div className="metric-label">Attacker Profit</div>
                <div className="metric-value profit-text">{attack.profit} ETH</div>
              </div>
              <div className="metric">
        ...
        <div className="profit-breakdown">
          ...
          <div className="profit-chart">
            ...
            <div className="chart-labels">
              <span>Front-run (60%)</span>
              <span>Back-run (40%)</span>
            </div>
          </div>
        </div>
      </div>

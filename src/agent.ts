...
// Prefer env var, then forta config helper, then fallback to Alchemy URL previously provided
const rpcUrl =
  process.env.JSON_RPC_URL ||
  getJsonRpcUrl() ||
  "https://eth-mainnet.g.alchemy.com/v2/HEwiJtBBUdWUa9LIaXaE7";
...
const MAX_POSITION_IN_BLOCK = 50;
...
const PRINT_CSV_PER_BLOCK = true; // Each block with its own header
      ...
      for (let j = i + 1; j < tradeSummaryArray.length; j++) {
        ...
        if (trade_maybe_attack_1 === trade_maybe_victim) {
          ...
          if (j < tradeSummaryArray.length - 1) {
            ...
            let is_flashbot_miner_msg =
              tx_victim.flashbotMiner == "" ? "Yes" : "No";
...
function provideHandleTransaction(
  ...
  return async function handleTransaction(tx: TransactionEvent) {
    ...
    if (
      !blocks_to_be_processed.includes(this_block) &&
      this_block > last_block_processed
    ) {
      ...
      getTransactionReceipt(tx.hash).then((receipt) => {
        ...
        for (let lg in receipt.logs) {
          ...
          for (let topic_num in log.topics) {
            ...
            if (topic === transferSig) {
              // Transfer()
              // Transfer has a single ammount
              const decoded_data = ethers.utils.defaultAbiCoder
        ...
        let gasPriceGwei = Web3.utils.fromWei(
          Web3.utils.toBN(tx.transaction.gasPrice),
          "Gwei"
        );
        ...
        let flashbotMiner = "";
        let loc = FlashbotMinerAddresses.indexOf(
          block_miner_dict[tx.blockNumber]
        );
        if (loc > -1) {
        ...
        if (PRINT_CSV_ALL) {
          alltxs_debug.push(saveTxObject);
        }
      }); //end of .then()
